...
namespace BusManagement.Pages.Admin.Schedules
{
    public class AddScheduleModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            {
                connection.Open();
                string query = "SELECT BusId, BusNumber FROM Buses WHERE Status='Available'";  // Assumes Bus table exists
        ...
        public IActionResult OnPost()
        {
            ...
            if (DepartureTime <= DateTime.Now)
                ModelState.AddModelError(nameof(DepartureTime), "Departure time must be in the future.");

            if (ArrivalTime <= DepartureTime)
                ModelState.AddModelError(nameof(ArrivalTime), "Arrival time must come after departure time.");
